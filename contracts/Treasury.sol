...
contract CitadelTreasury is CitadelAccessControlled, ITreasury {
    /* ========== DEPENDENCIES ========== */
    ...
    /**
     * @notice allow approved address to deposit an asset for CTDL
    ...
    function deposit(
        ...
        // mint CTDL needed and store amount of rewards for distribution
        send_ = value.sub(_profit);
        CTDL.mint(msg.sender, send_);
    ...
    /**
     * @notice allow approved address to burn CTDL for reserves
     * @param _amount uint256
     * @param _token address
     */
    function withdraw(uint256 _amount, address _token) external override {
        ...
        uint256 value = tokenValue(_token, _amount);
        CTDL.burnFrom(msg.sender, value);
    ...
    /**
     * @notice mint new CTDL using excess reserves
    ...
    function mint(address _recipient, uint256 _amount) external override {
        ...
        CTDL.mint(_recipient, _amount);
        emit Minted(msg.sender, _recipient, _amount);
    }

    /**
     * DEBT: The debt functions allow approved addresses to borrow treasury assets
     * or CTDL from the treasury, using sCTDL as collateral. This might allow an
     * sCTDL holder to provide CTDL liquidity without taking on the opportunity cost
    ...
    function incurDebt(uint256 _amount, address _token) external override {
        uint256 value;
        if (_token == address(CTDL)) {
            require(permissions[STATUS.CTDLDEBTOR][msg.sender], notApproved);
            value = _amount;
        } else {
        ...
        sCTDL.changeDebt(value, msg.sender, true);
        require(sCTDL.debtBalances(msg.sender) <= debtLimit[msg.sender], "Treasury: exceeds limit");
        totalDebt = totalDebt.add(value);

        if (_token == address(CTDL)) {
            CTDL.mint(msg.sender, value);
    ...
    function repayDebtWithReserve(uint256 _amount, address _token) external override {
        ...
        IERC20(_token).safeTransferFrom(msg.sender, address(this), _amount);
        uint256 value = tokenValue(_token, _amount);
        sCTDL.changeDebt(value, msg.sender, false);
    ...
    /**
     * @notice allow approved address to repay borrowed reserves with CTDL
     * @param _amount uint256
     */
    function repayDebtWithCTDL(uint256 _amount) external {
        require(permissions[STATUS.RESERVEDEBTOR][msg.sender] || permissions[STATUS.CTDLDEBTOR][msg.sender], notApproved);
        CTDL.burnFrom(msg.sender, _amount);
        sCTDL.changeDebt(_amount, msg.sender, false);
        totalDebt = totalDebt.sub(_amount);
        ohmDebt = ohmDebt.sub(_amount);
        emit RepayDebt(msg.sender, address(CTDL), _amount, _amount);
    }
    ...
    function enable(
        ...
        if (_status == STATUS.SCTDL) {
            sCTDL = IsCTDL(_address);
        } else {
    ...
    function execute(uint256 _index) external {
        ...
        if (info.managing == STATUS.SCTDL) {
            // 9
            sCTDL = IsCTDL(info.toPermit);
        } else {
    ...
    function excessReserves() public view override returns (uint256) {
        return totalReserves.sub(CTDL.totalSupply().sub(totalDebt));
    }

    /**
     * @notice returns CTDL valuation of asset
     * @param _token address
     * @param _amount uint256
     * @return value_ uint256
     */
    function tokenValue(address _token, uint256 _amount) public view override returns (uint256 value_) {
        value_ = _amount.mul(10**IERC20Metadata(address(CTDL)).decimals()).div(10**IERC20Metadata(_token).decimals());
    ...
    function baseSupply() external view override returns (uint256) {
        return CTDL.totalSupply() - ohmDebt;
    }
}
