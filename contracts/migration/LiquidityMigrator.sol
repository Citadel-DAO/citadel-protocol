...
contract LiquidityMigrator is CitadelAccessControlled {
    ...
    IERC20 internal immutable oldCTDL = IERC20(0x383518188C0C6d7730D91b2c03a03C837814a899);
    IERC20 internal immutable newCTDL = IERC20(0x64aa3364F17a4D01c6f1751Fd97C2BD3D7e7f1D5);
    IMigrator internal immutable migrator = IMigrator(0x184f3FAd8618a6F458C16bae63F70C426fE784B3);

    constructor(ICitadelAuthority _authority) CitadelAccessControlled(_authority) {}
    ...
    function migrateLP(
        ...
        uint256 initialNewCTDLBalance = newCTDL.balanceOf(address(this));
        ...
        (uint256 amountToken, uint256 amountCTDL) = routerFrom.removeLiquidity(
            token, 
            address(oldCTDL), 
            oldLPAmount,
            _minA, 
        ...
        // Migrate the V1 CTDL to V2 CTDL
        oldCTDL.approve(address(migrator), amountCTDL);
        migrator.migrate(amountCTDL, IMigrator.TYPE.UNSTAKED, IMigrator.TYPE.UNSTAKED);
        uint256 amountNewCTDL = newCTDL.balanceOf(address(this)).sub(initialNewCTDLBalance); // # V1 out != # V2 in

        // Add the V2 liquidity
        IERC20(token).approve(address(routerTo), amountToken);
        newCTDL.approve(address(routerTo), amountNewCTDL);
        ...
        newCTDL.safeTransfer(authority.governor(), newCTDL.balanceOf(address(this)));
        oldCTDL.safeTransfer(authority.governor(), oldCTDL.balanceOf(address(this)));
